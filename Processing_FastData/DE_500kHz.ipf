...
Static Function Process500kHz(FastDeflV,FastZSnsrV,SlowDeflection,SlowZSnsr)
	...
	string DeflNote=note(SlowDeflection)
	if(cmpstr(stringbykey("ZLVDTSens",note(SlowDeflection),":","\r"),"")==0)
			LVDTSens=1.6517e-06
			DeflNote=replacestringbykey("ZLVDtSens",DeflNote,"1.6517e-6",":","\r")

	else
			LVDTSens=str2num(stringbykey("ZLVDTSens",note(SlowDeflection),":","\r"))
	...
	wave Midpoints
	duplicate/free MidPoints MidPointsFast
	//killwaves Midpoints
	...
	print nameofwave(SlowDeflection)
	duplicate/o FastDeflV, $ReplaceString("Equil", nameofwave(SlowDeflection), "Fast")
	wave w1=$ReplaceString("Equil", nameofwave(SlowDeflection), "Fast")
	FastOp w1=(InVols)*w1+(w_coefs[0])
	duplicate/o FastZSnsrV $ReplaceString("Equil", nameofwave(SlowZSnsr), "Fast")
	wave w2=$ReplaceString("Equil", nameofwave(SlowZSnsr), "Fast")
		note/k w1 DeflNote
	note/k w2 DeflNote
